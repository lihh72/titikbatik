...
async def worker_context(tmp_path, monkeypatch):
    ...
    settings = Settings(
        ...
        generate_workflow_path=str((WORKFLOWS_ROOT / "generatebatik.json").resolve()),
        combine_workflow_path=str((WORKFLOWS_ROOT / "combinebatik.json").resolve()),
        video_workflow_path=str((WORKFLOWS_ROOT / "videobatik.json").resolve()),
