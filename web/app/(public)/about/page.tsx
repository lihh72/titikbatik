...
export default function AboutPage() {
  return (
    <main className="mx-auto max-w-7xl px-4 pb-8 sm:px-6 lg:px-8">
      <section className="glass-panel overflow-hidden rounded-[36px] p-6 sm:p-10 lg:p-14">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2">{architecture.map(({ title, text, icon: Icon }) => <article key={title} className="glass-soft rounded-[26px] p-6"><Icon size={21} className="text-[#ffad5d]" /><h2 className="mt-5 text-xl font-semibold">{title}</h2><p className="mt-3 text-sm leading-6 text-white/44">{text}</p></article>)}</div>
      </section>
      <section className="mt-7 grid gap-5 lg:grid-cols-[1.1fr_.9fr]"><div className="glass-panel rounded-[32px] p-7 sm:p-9"><div className="flex items-center gap-2 text-xs uppercase tracking-[.18em] text-[#ffb66c]"><ShieldCheck size={15} />Etika Penggunaan</div><h2 className="mt-4 text-3xl font-semibold tracking-tight">AI digunakan sebagai alat bantu, bukan pengganti perajin.</h2><div className="mt-6 space-y-4 text-sm leading-7 text-white/48"><p>Motif tradisional memiliki konteks sejarah, filosofi, dan aturan penggunaan yang perlu dihormati.</p><p>Hasil generatif dikurasi manusia sebelum digunakan untuk publikasi atau kepentingan lain.</p><p>Motif sakral, identitas komunitas, dan karya perajin tidak boleh diklaim tanpa atribusi dan verifikasi.</p></div></div><div className="glass-panel rounded-[32px] p-7 sm:p-9"><Layers3 size={21} className="text-[#ffad5d]" /><h2 className="mt-5 text-2xl font-semibold">Pengalaman publik yang sederhana</h2><p className="mt-4 text-sm leading-7 text-white/48">Antarmuka publik difokuskan pada pengalaman menjelajah koleksi. Detail teknis, prompt, riwayat produksi, dan kontrol AI disimpan pada lingkungan internal.</p><div className="mt-6 flex items-center gap-2 text-xs text-[#ffb363]"><Sparkles size={14} />Publikasi dilakukan setelah pemeriksaan admin</div></div></section>
    </main>
  );
}
