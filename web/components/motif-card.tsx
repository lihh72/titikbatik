...
export function MotifCard({ batik }: { batik: Batik }) {
  ...
  return <article className="overflow-hidden border border-white/10 bg-white/4">
    <Link href={`/gallery/${batik.id}`} className="block">
    ...
    <div className="flex gap-2 border-t border-white/8 p-3"><button onClick={() => toggleLike(id)} className={`grid h-9 w-9 place-items-center rounded-full border ${liked ? "border-red-300/30 bg-red-400/10 text-red-200" : "border-white/10 text-white/40"}`} title="Sukai"><Heart size={15} fill={liked ? "currentColor" : "none"} /></button><button onClick={() => toggleBookmark(id)} className={`grid h-9 w-9 place-items-center rounded-full border ${bookmarked ? "border-[#ff9d42]/35 bg-[#ff9d42]/10 text-[#ffb66c]" : "border-white/10 text-white/40"}`} title="Simpan"><Bookmark size={15} fill={bookmarked ? "currentColor" : "none"} /></button><Link href={`/gallery/${batik.id}`} className="ml-auto flex items-center px-3 text-xs text-[#ffb66c]">Lihat detail</Link></div>
  </article>;
}
